...
export default function VideoTestimonialSection({ data }) {
  const [currentPlayingVideoRef, setCurrentPlayingVideoRef] = useState(null); // Track currently playing video
  useEffect(() => {
    setCurrentPlayingVideoRef(null);
  }, []);
  return (
    <section className="video-testimonial-section bg-[##FDF3DC]">
      <div className="section-bg bg-[##FDF3DC] bg-[url('/pages/home/papertexture.png')] bg-size-[240px_240px] bg-repeat py-25 lg:py-35">
        <Container>
            {/*=============================================================================Section head*/}
          <div className="section-head max-w-245 mx-auto  flex flex-col items-center px-2.5 ">
            <h2 className="section-title text-center text-neutral-950">
              {data?.headline}
            </h2>
            <p className="section-subtitle text-neutral-700 text-center text-[14px] lg:text-[18px] font-normal mt-4">
              {data?.description}
            </p>
          </div>
          {/*=============================================================================video carousel */}
          <div className="video-testimonial-carousel-box mt-12.5 lg:mt-20">
            <Swiper
              slidesPerView={1.2}
              spaceBetween={20}
              pagination={{
              ...
              {data?.testimonial_card?.map((item, index) => (
                <SwiperSlide key={index}>
                  <div
                    ...
                    className="group cursor-pointer relative w-full max-w-full mx-auto aspect-378/500 rounded-xl overflow-hidden "
                  >
                    <video
                    ...
                    <button
                      className={`cursor-pointer absolute z-20 bottom-[50%] left-[50%] translate-x-[-50%] rounded-md group-hover:scale-110 duration-300 ${videoRefs?.current[index]?.paused ? "group-hover:drop-shadow-[0px_0px_13px_#E64027]" : ""} `}
                    >
                    ...
                    <div
                      className={`${videoRefs?.current[index]?.paused ? "opacity-100" : "opacity-0"} duration-500 absolute inset-0 z-10 bg-[linear-gradient(0deg,rgba(13,17,22,0.6)_0%,rgba(13,17,22,0.6)_100%)]`}
                    ></div>
                  </div>
                </SwiperSlide>
              ))}
