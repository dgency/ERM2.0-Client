...
//import CardBgVideo from "@/components/optimized/CardBgVideo";
import CardBgVideo from "@/components/optimized/elements/CardBgVideo";
...
export default function BookingMax() {
  ...
  const [before, highlightedWithQuotes] = data
    ? data?.bookingmax?.headline?.split("'")
    : [];
  ...
  const fetchData = async () => {
    try {
      ...
      console.log("fetching booking max data from:", url);
      ...
      <div className="relative py-[100px] md:py-[140px]">
        ...
        <div className=" px-2.5 flex flex-col items-center mb-[50px] md:mb-[80px] relative z-10">
          <h1 className="text-[clamp(50px,15.6vw,160px)]  font-[400] text-neutral-50 mb-1">
            {before}
            {highlighted && (
              <span className="text-primary-500 font-bold">{highlighted}</span>
            )}
          </h1>
          <UnderlineHeadline
            text={data?.bookingmax?.description}
            text_light={true}
            text_center={true}
          />
        </div>
            ...
            {data?.bookingmax?.other_card?.map((card, index) => (
              ...
              >
                ...
                <div className="relative z-20">
                  <h3 className="text-neutral-50 font-[600] text-[20px] md:text-[24px] mb-[10px]">
                    {card?.title}
                  </h3>
                ...
                <div className="booking-max-bg-video absolute inset-0 z-0 ">
                  ...
                  <CardBgVideo video_url={card?.video_url} />
                </div>
