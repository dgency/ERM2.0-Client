  ...
  return (
    <div className="relative max-w-full  rounded-2xl mx-auto aspect-[300/400] xs:aspect-[398/295]  md:aspect-[1584/700] bg-[rgba(22,26,30,0.90)]  ">
      ...
      <div className="absolute inset-0 max-card-video-bg ">
        <CardBgVideo video_url={data?.intro_video_url} border_radious={true} />
      </div>
      ...
      <div className="flex items-end h-full pb-[30px] md:pb-[50px] px-5 md:px-[32px] relative z-20">
        <div
          className={`lg:flex gap-6 items-end w-full ${data?.title ? "justify-between " : "justify-end"}`}
        >
          {data?.title && (
            <div className="max-w-[700px]">
              <span className="block text-[20px] sm:text-[24px] text-neutral-50 font-[600] mb-2">
                {data?.title}
              </span>
      ...
      {/* Fullscreen Overlay */}
      {open && (
        <VideoPopUp video_url={data?.video_url} open={open} setOpen={setOpen} />
      )}
    </div>
  );
}
