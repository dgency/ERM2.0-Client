  ...
  return (
    <div className="">
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            className="fixed inset-0 z-1000 mx-2.5 md:mx-0 aspect-videomd:aspect-auto  md:h-[80%] md:w-[80%] top-[50%] md:left-[50%] md:translate-x-[-50%] translate-y-[-50%] rounded-[20px]"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            ref={overlayRef}
          >
            ...
            <button
              onClick={handleClose}
              className="group absolute cursor-pointer top-4 right-4 md:top-6 md:right-6 rounded-full bg-[#ff482c57] hover:bg-transparent hover:border-primary-400 border-transparent border duration-500 backdrop-blur-[20px] p-2 z-50"
            >
